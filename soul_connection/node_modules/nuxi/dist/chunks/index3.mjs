...
import process, { platform, hrtime, execPath, execArgv } from 'node:process';
import { writeFileSync, statSync, readFileSync, appendFileSync, createReadStream, createWriteStream } from 'node:fs';
import tty__default from 'node:tty';
import path from 'node:path';
import { c as crossSpawn, n as npmRunPathEnv, o as onExit } from '../shared/nuxi.6ce4ecc8.mjs';
import { setTimeout, scheduler, setImmediate } from 'node:timers/promises';
import { constants } from 'node:os';
import { once, addAbortListener, EventEmitter, on, setMaxListeners } from 'node:events';
import { serialize } from 'node:v8';
import { Transform, getDefaultHighWaterMark, PassThrough, Readable, Writable, Duplex } from 'node:stream';
...
const hasColors = tty__default.WriteStream.prototype.hasColors();
...
const getFinalIcon = ({failed, reject}) => {
	if (!failed) {
		return figures$1.tick;
	}

	return reject ? figures$1.cross : figures$1.warning;
};
...
export { ExecaError, ExecaSyncError, execa, parseCommandString };
